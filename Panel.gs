...
const Panel = {
    ...
    try {
      ...
      if (applications.length === 0) {
        SpreadsheetApp.getUi().alert(
          ...
          'No tenant applications found in the responses sheet. Make sure the Google Form is linked and has responses.',
  ...
  /**
   * Get tenant applications from form responses
   * @private
   */
  _getTenantApplications() {
    try {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        // Skip if already processed (check if there's a "Processed" column with data)
        ...
        const application = {
          ...
          fullName: this._getFieldValue(row, headerMap.fullName) || 'Unknown',
      ...
      console.log(`Found ${applications.length} unprocessed applications`);
      return applications;
  ...
  /**
   * Create a mapping between form headers and our expected fields
   * @private
   */
  _createHeaderMap(headers) {
    ...
    headers.forEach((header, index) => {
      ...
      if (lowerHeader === 'full name' || lowerHeader.startsWith('full name')) {
      ...
      } else if (lowerHeader.includes('move-in date') || lowerHeader.includes('move in')) {
        ...
        map.employer = index;
      } else if (lowerHeader.includes('monthly income') || lowerHeader.includes('income')) {
        map.monthlyIncome = index;
      } else if (lowerHeader.includes('reference 1')) {
        map.reference1 = index;
      } else if (lowerHeader.includes('reference 2')) {
      ...
      } else if (lowerHeader.includes('proof of income')) {
        map.proofOfIncome = index;
      }
    });
...
<body>
            ...
            <div class="application-card" id="app-${index}">
                ...
                <div class="app-details">
                    ...
                    <div>
                        ...
                        <div class="app-field"><strong>Monthly Income:</strong> ${app.monthlyIncome}</div>
    ...
    <script>
        ...
        function rejectApplication(index) {
            if (confirm('Are you sure you want to reject this application? This action cannot be undone.')) {
                showStatus('Application rejected.', 'error');
  ...
  /**
   * Mark application as processed in the responses sheet
   * @private
   */
  _markApplicationAsProcessed(timestamp, applicantName) {
    try {
      ...
      // Find and mark the specific application as processed
      for (let i = 1; i < data.length; i++) {
        if (data[i][0].toString() === timestamp.toString()) {
          responseSheet.getRange(i + 1, processedCol + 1).setValue(`Approved - ${new Date().toLocaleDateString()}`);
          break;
        }
      }
